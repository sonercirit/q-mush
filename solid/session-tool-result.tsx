...
function renderShellStream(options: {
  ...
  return (
    <section
      ...
      class={`overflow-hidden rounded-lg border ${stderr ? "border-rose-300/20 bg-rose-950/20" : "border-cyan-300/20 bg-cyan-950/20"}`}
    >
      <p class="border-b border-white/10 px-3 py-2 font-mono text-[0.65rem] font-semibold tracking-wider uppercase">
        <span class={stderr ? "text-rose-300" : "text-cyan-300"}>
          {options.kind}
        </span>
      </p>
...
function renderShellOutput(content: string): JSX.Element | undefined {
  ...
  return (
    <div class="space-y-2">
      ...
      {output.status.length === 0 ? null : (
        <p
          class={`rounded-lg border px-3 py-2 font-mono text-xs ${success ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-300" : "border-rose-300/20 bg-rose-300/10 text-rose-300"}`}
...
function renderReadOutput(options: ToolOutputOptions): JSX.Element {
