...
- Bare-metal file tools resolve relative paths against the runner workspace but
  ...
  `read` pages its source. Directory browsing escapes the session workspace,
  returns bounded directory-only metadata, times out stalls, maps HTTP
  ...
  ending an active shell. OpenAI API-key and OAuth requests prefer Responses
  WebSockets, falling back to HTTP streaming; OpenRouter and generic endpoints
  stream chat completions, Anthropic-format endpoints Messages events. OpenAI
  OAuth refreshes its token bundle before expiry. Session creation needs an
  explicit model ID. Catalogs: OpenAI `/v1/models`, OpenRouter
...
- Tools share a 30-minute limit, stated once in prompt/picker; `parallel` shares
  a budget. Sleep uses it; ask_questions waits. Shell has a runner timer; each
