...
import { createHarness } from "./render-debug-dom-helpers.tsx";
...
test("automatic render instrumentation observes transcript filter updates generically", () => {
  const detail: AgentSessionDetail = {
    ...TEST_SESSION_DETAIL,
    messages: [
      transcriptTestMessage("user-debug", "Visible prompt", "user", 2),
      transcriptTestMessage(
        "thinking-debug",
        "Filtered thought",
        "thinking",
  ...
  const thinkingMessage = transcriptMessageElement(
    container,
    "Filtered thought",
    "Thinking",
  );
  ...
  transcriptFilter(container).click();
  harness.observer.records([
    createMutationRecord({
      removedNodes: [thinkingMessage],
      target: transcript,
      type: "childList",
    }),
  ]);
  ...
  expect(container.textContent).not.toContain("Filtered thought");
  const transcriptHighlight = [
    ...document.querySelectorAll(".render-debug-highlight"),
  ].find((highlight) => highlight.textContent.startsWith("ul"));
  expect(transcriptHighlight?.classList).toContain(
    "render-debug-highlight--remove",
  );
  expect(
    document.querySelectorAll(".render-debug-highlight").length,
  ).toBeLessThanOrEqual(100);
  expect(
    document.querySelector("#render-debug-overlay")?.textContent,
  ).not.toContain("Filtered thought");
  harness.debug.detach();
});
...
test("filtering streamed categories preserves placeholders and canonical order", () => {
  ...
  const streamedAnswer = transcriptMessageElement(
    container,
    "Visible answer",
    "Agent",
  );
  ...
  expect(
    transcriptMessageElement(container, "Visible answer continues", "Agent"),
  ).not.toBe(streamedAnswer);
