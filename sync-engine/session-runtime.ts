...
export interface SessionRuntimes {
  ...
  forcePark(
    scope: RestartScope,
    initialPersistence?: Promise<unknown>,
...
export function createSessionRuntimes(
  now: () => number = Date.now,
): SessionRuntimes {
  ...
  const requestRestart = (
    ...
    const existing =
      scope.kind === "server"
        ? drainingServer
        : drainingRunners.get(scope.runnerId);
    ...
    if (existing === undefined)
      applyRequest(
        ...
        request,
        (value) => {
          drainingServer = value;
        },
        (runnerId, value) => {
          drainingRunners.set(runnerId, value);
        },
      );
    const affected = [...activeRuntimes.values()].filter(({ runnerId }) =>
  ...
  const runtimes: SessionRuntimes = {
    ...
    accepts(runnerId: string): boolean {
      return drainingServer === undefined && !drainingRunners.has(runnerId);
    },
    ...
    requestDrain(
      ...
      const { affected, persistence } = requestRestart(
        scope,
        restartId,
        durable,
      );
      ...
      if (runnerId === undefined) {
        ...
        for (const runtime of activeRuntimes.values()) {
          ...
          if (
            ...
            runnerGate !== BLOCKED_RUNNER_RESTART
          ) {
