...
export class WorkspaceStore {
  ...
  create(
    ...
    return { id, isDefault: false, name: normalizedName };
  }
  ...
  createDefault(userId: string, now: number): WorkspaceSummary {
    ...
    this.#database
      ...
      .run();
    return { id, isDefault: true, name: DEFAULT_WORKSPACE_NAME };
  }
  ...
  defaultForUser(userId: string): WorkspaceSummary | undefined {
    return this.#database
      ...
      .from(workspaces)
      .where(
        and(activeWorkspaceCondition(userId), eq(workspaces.isDefault, true)),
      )
      .get();
  }
  ...
  rename(
    ...
    const normalizedName = normalizeWorkspaceName(name);
    if (normalizedName === undefined || workspaceId === GLOBAL_WORKSPACE_ID) {
      return undefined;
    }
    ...
    return this.#database.transaction((transaction) => {
      ...
      if (workspace.isDefault) {
        transaction
          ...
          .where(eq(workspaces.id, replacement.id))
          .run();
      }
      return "removed";
    });
