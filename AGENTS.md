...
- `sync-engine/sessions.ts` and `session-store.ts` persist coding sessions. User
  ...
  reuse. Evicting any pending tool update, including a terminal tombstone, must
  request a snapshot: committing an undelivered terminal locally does not clear
  the controller's previously rendered running row. Epochs stay monotonic while
