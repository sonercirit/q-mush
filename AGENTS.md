...
- CI: tests, static checks, build, whitespace on Bun 1.3.14/frozen lockfile.

## Architecture

- Production has four workspaces: `solid` owns browser UI, `sync-engine` the Bun
  ...
  workspace; only `scripts` imports `scripts`.
- `server.ts` serves Vite's in-memory browser JS/Tailwind CSS. Authenticated
  ...
  writes it, while plain `dev` restarts only from it. `runner-executable.ts`
  fingerprints runner source/compiler, builds privately, caches in memory,
  serves `/runner/executable`. Restarts drain active steps and queue new work,
  so sessions may request their own restart. Text handlers precompress once,
  negotiating zstd, Brotli, gzip, deflate; `/favicon.svg` revalidates with ETag.
...
- `sync-engine/auth.ts` implements Google OpenID Connect (authorization code
  - PKCE) with HttpOnly state/verifier cookies, fetching the basic profile and
    ...
    SQLite upserts users and persists seven-day sessions. Primary keys are
    ...
    use Chromium/Tailwind and production state/UI mutations, never synthetic
...
- `sync-engine/runner-store.ts` persists runner registrations in `runners`: one
  ...
  WebSockets, check updates at startup/five-minute intervals, recheck via
  handshake version after restarts, replacing an older socket on reconnect.
  Updates use source/compiler ETags and SHA-256, atomically replace and restart
  the executable; development restarts drain active sessions first. Reinstalling
...
- `sync-engine/sessions.ts` and `session-store.ts` persist coding sessions. User
  ...
  pickers use canonical schemas. `read_session` spans selected transcript
  categories and definitions with positional record pagination;
  ...
  2+ calls on four ordered workers and propagates cancellation.
  ...
  breaks; session lists page by ten. Live sessions use
  `solid/realtime-client.ts`, `solid/session-client.tsx`,
  `solid/session-controller.ts`: model deltas combine once per frame; other
  events are immediate, unchanged snapshots suppress notifications, keyed
  ...
  bottom-pinned transcripts follow live output. `agent-model-discovery.ts`
...
- `openai.ts`, `openrouter.ts`, and `generic-provider.ts` implement model
  ...
  AES-256-GCM encryption; API responses expose metadata only; one credential may
...
- `solid/client.tsx` is the browser entry; `solid/pages.tsx` owns SSR shells,
  ...
  for string rendering and a Happy DOM project for post-mount reactivity; run it
...
- `tsconfig.json` configures strict, no-emit, bundler-style checking with unused
  and unreachable diagnostics. Library declaration checking is off — Drizzle
  publishes optional cross-dialect declarations that fail here; app source stays
  checked; re-enable after an upstream Drizzle fix.
- `eslint.config.ts` uses type-aware strict/stylistic `typescript-eslint`
  ...
  production/browser-test imports of `solid/styles.css`) are rejected.
...
- Bare-metal file tools resolve relative paths against the runner workspace but
  ...
  `read` pages source. Directory browsing escapes the session workspace, returns
  bounded directory-only metadata, times out stalls, maps HTTP cancellation to a
  browse error, and propagates agent-tool cancellation. Stopping a session
...
- Tools use persisted user settings, defaulting to 30 minutes and 20,000 Unicode
  ...
  deadline, sleep, skills/session tools, and final model-facing result bound;
  changes apply next run and realtime updates stay user-scoped. Loading clears
  its timer and aborts its signal on settlement. `parallel` shares one budget;
