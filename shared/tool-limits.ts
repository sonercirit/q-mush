...
// not repeat them.
export const SHARED_TOOL_LIMITS_STATEMENT = `Every tool call is limited to ${String(MAXIMUM_TOOL_EXECUTION_MINUTES)} minutes; longer runs are canceled. Tool output is limited to ${MAXIMUM_TOOL_OUTPUT_LINES.toLocaleString("en-US")} lines or ${String(MAXIMUM_TOOL_OUTPUT_KILOBYTES)}KB and larger outputs are truncated or spilled to a file. A parallel batch shares its single call's time budget. ask_questions pauses the session instead, so the wait for an answer has no time limit.`;
