...
- Strict-TypeScript ESM Bun/SolidJS project; tests live under `test/` (no
  `src`). `/` is the homepage, `/app` the app.
...
- `sync-engine/auth.ts` implements Google OpenID Connect (authorization code
  - PKCE) with HttpOnly state/verifier cookies, fetching the basic profile and
    ...
    fields; every table carries created/updated timestamps, actor IDs, and
    `isDeleted`. `shared/database.ts` applies committed `drizzle/` migrations on
    open; `sync-engine/index.ts` injects the persistent connection, and the auth
    factory falls back to in-memory SQLite. Shared PKCE, provider parsing, and
