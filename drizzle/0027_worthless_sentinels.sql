...
CREATE UNIQUE INDEX `agent_session_operations_active_generation_unique` ON `agent_session_operations` (`session_id`,`execution_generation`) WHERE NOT "agent_session_operations"."is_deleted";--> statement-breakpoint
...
CREATE TABLE `__new_agent_sessions` (
	...
	`shutdown_interrupted_handoff` text,
	...
	`title` text NOT NULL,
	`tools` text DEFAULT '["page_fetch","read","explain_file","bash","edit","write","parallel","brave_search","ask_questions","sleep","spawn_session","browse_runner_directories","list_runners","list_sessions","get_session_options","read_session","reassign_session","send_to_session","continue_session","compact_session","steer_session","stop_session"]' NOT NULL,
