...
describe("agent conversation compaction", () => {
  test("uses a 95% threshold only when the context limit is known", () => {
    expect(shouldCompactContext(94_999, 100_000)).toBeFalse();
    expect(shouldCompactContext(95_000, 100_000)).toBeTrue();
    expect(shouldCompactContext(100_000, null)).toBeFalse();
  });
