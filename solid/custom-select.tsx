...
function optionContent(option: CustomSelectOption): JSX.Element {
  return (
...
export function renderCustomSelect(options: CustomSelectOptions): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        type="button"
      >
        <span class="flex min-w-0 flex-1" id={`${options.id}-value`}>
          {selected === undefined
            ? options.emptyLabel
        ...
        <span
          aria-hidden="true"
          class={`shrink-0 text-slate-500 transition ${options.open ? "rotate-180" : ""}`}
        >
          ▾
        </span>
      </button>
