...
export function createWorkspaceController(
  ...
  const load = async (): Promise<void> => {
    ...
    const workspaces = result === undefined ? undefined : readWorkspaces(result);
    if (workspaces !== undefined && (selectedId() === GLOBAL_WORKSPACE_ID ||
      !workspaces.workspaces.some(({ id }) => id === selectedId()))) {
      controller.select(workspaces.defaultWorkspaceId);
    }
  };
  const requestAndReload = async (
    path: string, init: RequestInit, pending: Partial<WorkspaceViewState>,
    settled: Partial<WorkspaceViewState>, errorMessage: string,
  ): Promise<void> => {
    await state.mutation(path, init, request,
      () => ({ ...settled, error: errorMessage }),
      { ...pending, error: undefined }, settled, load);
  };
  const update = async (workspaceId: string, name: string | undefined): Promise<void> => {
    await requestAndReload(
      `${WORKSPACES_PATH}/${encodeURIComponent(workspaceId)}`,
      name === undefined ? { method: "DELETE" } : jsonRequestInit({ name }, "PATCH"),
      name === undefined ? { removingId: workspaceId } : { renamingId: workspaceId },
      name === undefined ? { removingId: undefined, settingDefaultId: undefined } : { renamingId: undefined },
      name === undefined
  ...
  const controller: WorkspaceController = {
    ...
    view: state.accessor,
    async create(name) {
      await requestAndReload(WORKSPACES_PATH, jsonRequestInit({ name }, "POST"),
        { creating: true }, { creating: false }, "We could not create that workspace.");
    },
    load,
    ...
    async setDefault(workspaceId) {
      await requestAndReload(workspaceDefaultPath(workspaceId), { method: "POST" },
