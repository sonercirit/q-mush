...
export interface DrizzleAuthStore {
  createSession(
    token: string,
...
export function createDrizzleAuthStore(
  database: AppDatabase,
  generateId: IdGenerator = createUuidV7,
): DrizzleAuthStore {
  const softDeleteSessions = (
    ...
    database
      .update(sessions)
      .set({ isDeleted: true, updatedAt: new Date(now), updatedById })
      .where(condition)
      .run();
  };

  return {
    createSession(token, profile, expiresAt, now) {
