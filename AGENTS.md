...
- Bare-metal file tools resolve relative paths against the runner workspace but
  ...
  pre-creation, and mismatched-ID frames. Since 60 minutes does not bound
  provider-controlled ID size/rate, retain at most Bun's 16 MiB inbound-frame
  default of ID payload, then retire rather than evict from the stale-frame
  fence; observed OpenAI IDs are ~53 bytes. After ID-less admission, skip
  retained IDs until a new ID. Fenced watchdog failures abort without replaying
  tools. Other interruptions and provider errors retry before persistence;
  replays reset partial UI, and exhausted WebSockets use HTTP. Permanent errors
