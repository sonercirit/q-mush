...
import { inlineCodeClasses, renderHighlightedCode } from "./session-syntax.tsx";
...
function closingInline(
  ...
  marker: string,
  index: number,
  createNode: (content: string) => JsxNode,
...
function markedInline(text: string, index: number): InlineRender | undefined {
  if (text[index] === "`" && text[index + 1] !== "`") {
    return closingInline(text, "`", index, (content) => (
      <code className={inlineCodeClasses()}>{content}</code>
    ));
  }
  ...
  if (text[index] === "*" || text[index] === "_") {
    ...
    return closingInline(text, inlineMarker, index, (content) => (
      <em className="text-slate-100 italic">{renderInline(content)}</em>
    ));
  }
...
function renderInline(text: string): readonly JsxNode[] {
  const nodes: JsxNode[] = [];
  let plain = "";
  let index = 0;
  const flush = () => {
...
function parseMarkdownBlocks(content: string): readonly MarkdownBlock[] {
  ...
  while (index < lines.length) {
    ...
    const parsed = appendParagraphLines(
      lines,
      index,
      isRawTranscriptLine(line),
    );
    blocks.push({
      text: parsed.text,
      type: isRawTranscriptLine(line) ? "raw" : "paragraph",
    });
...
function renderMarkdownList(block: MarkdownListBlock): JsxNode {
  const children = block.items.map((item) => (
    <li>
      {item.checked === undefined ? null : (
        <span
          aria-hidden="true"
          className={item.checked ? "text-emerald-300" : "text-slate-500"}
        >
          {item.checked ? "☑ " : "☐ "}
        </span>
      )}
  ...
  return block.ordered ? (
    <ol className="list-decimal space-y-1 pl-5" start={block.start}>
      {children}
    </ol>
  ) : (
...
function renderMarkdownBlock(block: MarkdownBlock): JsxNode {
  switch (block.type) {
    case "code":
      return renderHighlightedCode(block.code, block.language);
    case "heading":
      return (
        <h2 className={headingClasses(block.level)}>
    ...
    case "quote":
      return (
        <blockquote className="border-l-2 border-cyan-300/40 pl-3 text-slate-400 italic">
          {renderInline(block.text)}
        </blockquote>
      );
    case "raw":
...
export function renderMarkdown(content: string): JsxNode {
  return (
