...
### Stage 4 — Always peer-first transport and discovery

**Standalone value:** ordinary runner replication and commands are endpoint-to-
endpoint even while the engine is healthy.

Scope:

- Implement authenticated runner WebSocket frames: loopback, pinned-TLS
  LAN/manual/VPN, runner mDNS with opaque metadata, and WebRTC where NAT needs
  it.
- Use reachable runner/manual/cached signaling by default. Add paid engine
  rendezvous and endpoint-encrypted relay only as explicit fallback; provide
  configuration hooks for user-operated routes.
- Enforce local/direct/LAN/VPN/WebRTC first. Add no engine endpoint that
  terminates, inspects, authorizes, stores-and-forwards, or re-emits ordinary
  runner operations, blobs, commands, receipts, or streams. Backup subscription
  stays a distinct endpoint pair/channel.
- Make Solid realtime a runner view/query/command/live multiplexer. Same-origin
  tabs may elect transport through `BroadcastChannel` but do not exchange
  durable operation frames.
- Add route diagnostics, grant intersection, revocation priority, flow control,
  quotas, and path assertions.

Exit criteria:

- Healthy-engine captures show direct runner traffic and separate backup links,
  with zero engine broker hops. Direct failure yields labeled paid/user relay or
  `No route`.
- Blocking engine data sockets changes no established runner flow. Anonymous/
  free direct/manual paths continue; managed rendezvous/relay enforces paid
  tier.
- Loopback/LAN/manual/WebRTC/NAT/relay/revocation/version cases are tested.
- Discovery leaks no account, workspace, path, session, or provider metadata.
...
- If selected, ship self-hosted connectivity for non-paid modes without changing
  managed entitlement or direct-first policy.
...
- Default steady state issues engine requests only for explicit Google
  identity/recovery, independent default backup, paid rendezvous/relay, or an
  update source—never ordinary runner traffic or provider credentials.
...
Topology instrumentation records endpoint pairs/bytes by channel. A healthy
engine must see only its own backup/rendezvous/opaque-relay traffic, never
ordinary broker flow. Browser protocol tests prove absence of replica/frontier/
ack capability. Restore tests physically remove all runner storage and ignore
browser caches.
...
operations, snapshots, both backup tiers, P2P/relay captures, logs, crashes,
exports, and tools. Credential tests separately inspect target ciphertext and
verify no engine request. OpenAI device and OpenRouter runner callback tests
include denial, expiry, slow-down, replay, mix-up, cancellation, and route
removal. Full suites must not overlap repository-policy scans that create probe
files.
