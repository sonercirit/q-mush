...
function renderSyntaxToken(token: SyntaxToken): JsxNode {
  return token.kind === undefined ? (
    token.text
  ) : (
    <span className={SYNTAX_TOKEN_CLASSES[token.kind]}>{token.text}</span>
  );
}

function renderJsonPrimitive(value: JsonPrimitive): JsxNode {
...
function renderJsonLines(value: JsonValue, depth = 0): readonly JsxNode[] {
  ...
  return [
    open,
    "\n",
    ...entries.flatMap(([key, item], index): readonly JsxNode[] => [
      "  ".repeat(depth + 1),
      ...(key === undefined
        ? []
        : [
...
function highlightedSource(code: string): readonly JsxNode[] {
  return code
    .split("\n")
    .flatMap((line, index) => [
      ...(index === 0 ? [] : ["\n"]),
...
export function renderHighlightedCode(
  ...
  return (
    <pre className={classes} data-language={normalized}>
...
export function renderStructuredCode(
  ...
  return json === undefined ? (
    <pre className={classes}>{content}</pre>
  ) : (
    <pre className={classes} data-language="json">
