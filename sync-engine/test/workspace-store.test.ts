...
function expectForeignWorkspaceHidden(
...
function expectWorkspaceInUse(store: WorkspaceStore, updatedAt: number): void {
  expect(store.remove(USER_ID, SECOND_ID, updatedAt)).toBe("workspace_in_use");
}

function insertWorkspaceScope(
  database: ReturnType<typeof createDatabase>,
  audit: ReturnType<typeof createdAuditFields>,
  target: "credential" | "runner",
): void {
  if (target === "runner") {
    database
...
describe("workspace store", () => {
  test("creates a default and supports owned CRUD", () => {
    ...
    expectForeignWorkspaceHidden(store, database);
  });
  ...
  test("blocks deletion while sessions or scoped connections reference a workspace", () => {
    ...
    database
      ...
      .run();
    insertWorkspaceScope(database, audit, "runner");
    expectWorkspaceInUse(store, NOW + 2);
    database.update(runnerWorkspaces).set({ isDeleted: true }).run();
    insertWorkspaceScope(database, audit, "credential");

    expectWorkspaceInUse(store, NOW + 3);
    database
    ...
    expectWorkspaceInUse(store, NOW + 4);
    database.$client.close();
  });
});
